...
Fridge (sensor.temp_sensor_main_fridge): normal 35 to 39F. Early warning above 43F for 10 min. Ceiling alert above 45F for 20 min. Runaway cold below 33F for 20 min.
...
Batteries (sensor.battery_sensor_main_fridge, sensor.inkbird_temperature_sensor_battery): below 10 percent. The low threshold is intentional because these small batteries fall off quickly once past 20 percent.
...
| Master Updates Notification | Notifies when any update entity becomes available or is completed, using a green light flash for critical infrastructure (HA Core, OS, Supervisor, Apps, UniFi gear) and royalblue for everything else.
