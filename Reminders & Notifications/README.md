...
| Parcel box delivery notification | Plays chime and TTS when a package is delivered to the parcel box. Prevents repeat alerts using a dashboard indicator boolean. Allows manual reset and auto-reset at 6:00 AM. Adds actionable notification buttons to reset or dismiss on Scott's phone only. CAST PATH AND FAILURE HARDENING, 2026-08-30: every parcel box run that touched a speaker failed with a Music Assistant error about the Office Speaker refusing to launch the cast app, aborting before the TTS, the flag, and both notifications. The four speaker targets were switched from the Music Assistant player entities to their native Cast counterparts, every media_player step now carries continue_on_error, the dashboard indicator is set first, and the TTS media_content_type was corrected from audio/mp3 to provider. TO REVERT THE TARGETS, set the entity_id list on the volume_set step and all three play_media steps back to media_player.bedroom_speaker, media_player.dining_room_speaker, media_player.office_speaker, media_player.sunroom_speaker. SPEAKER SET REDUCED, 2026-08-30: announcements now go to the Bedroom and Dining Room Cast speakers only. Casting to four individual Cast devices played them slightly out of sync, which was confusing to listen to. Office and Sunroom were dropped. TTS TRUNCATION TEST, 2026-08-30: the TTS step was converted from media_player.play_media to tts.speak with cache true and preferred_sample_rate 44100, to test whether the sample rate is why the Cast speakers cut the sentence short. If this does not fix it, shorten the message instead. |
